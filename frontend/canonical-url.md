...
Возможно есть более удачное регулярное выражение, чтобы не добавлять отдельную строку для главной страницы. Тестирование проводилось для адресов `http://localhost:64524/canonical/index/` и `http://localhost:64524/index/`. Причем редирект хотелось получить только один.
